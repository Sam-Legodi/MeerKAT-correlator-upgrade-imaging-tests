...
LOW_LO_HZ_REQ, LOW_HI_HZ_REQ = _env_hz_pair("MCI_TCLEAN_LOWBAND_HZ", (int(round(8.98e8)), int(round(1.00e9))))
HIGH_LO_HZ_REQ, HIGH_HI_HZ_REQ = _env_hz_pair("MCI_TCLEAN_HIGHBAND_HZ", (int(round(1.46e9)), int(round(1.70e9))))

# Default imaging parameters (edit as needed)
MSFILE     = os.environ.get("MCI_TCLEAN_MSFILE", "")  # if non-empty, CLI args are ignored
...
FIELDNAME  = os.environ.get("MCI_TCLEAN_FIELD", "J2147-8132")
CELL       = '1.2arcsec'
DATACOL    = os.environ.get("MCI_TCLEAN_DATACOL", "DATA")  # "corrected" or "data"
IMSIZE     = 4096
NITER      = 10000
STOKES     = 'I'
GAIN       = 0.1
...
IMAGE_SCANS     = _env_bool("MCI_TCLEAN_IMAGE_SCANS", True)  # leave this True to enable per-scan imaging
...
def _parse_scan_list(scans):
    if not scans:
        return None
    out = []
    for token in str(scans).replace(",", " ").split():
        try:
            out.append(int(token))
        except Exception:
            print("Ignoring invalid scan id '{}'".format(token))
    return out
    ...
    for ms in targets:
        ...
        if IMAGE_SCANS:
            if requested_scan_ids is not None:
                scan_ids = requested_scan_ids
            elif "1757723806" in ms:
                scan_ids = [3, 5, 7, 9]
