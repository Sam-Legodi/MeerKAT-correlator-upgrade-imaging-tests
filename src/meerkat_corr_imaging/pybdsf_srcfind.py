...
def find_sources(
    ...
    *,
    ...
    """
    Run PyBDSF on a single FITS image and export GAUL catalogues, model and RMS images.

    The GAUL catalogue is written in both FITS and ASCII formats.  The FITS
    product is the input consumed by the pipeline's cross-matching step; the
    ASCII product is retained for convenient inspection and compatibility with
    earlier runs.
    ...
    try:
        ...
        if not wrote_temp:
            ...
            # ambiguous in current releases (`frequency` vs `frequency_sp`).
            kwargs["frequency"] = float(freq_hz)
        ...
        srcfilename_fits = os.path.join(outdir, base + "-source-cat.fits")
        srcfilename_ascii = os.path.join(outdir, base + "-source-cat.txt")
        ...
        img.write_catalog(
            outfile=srcfilename_fits,
            clobber=True,
            format="fits",
            catalog_type="gaul",
        )
        img.write_catalog(
            outfile=srcfilename_ascii,
            clobber=True,
            format="ascii",
            catalog_type="gaul",
        )
        ...
        logger.info("Exported FITS GAUL catalog to %s", srcfilename_fits)
        logger.info("Exported ASCII GAUL catalog to %s", srcfilename_ascii)
...
def parse_args(argv: Optional[List[str]] = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Optional prefix to prepend to the sanitized FITS stem for the output "
...
def determine_frequency(
    ...
    """
    Determine an explicit frequency override for the image.

    File-list values are true per-image overrides. The global --freq-* values
    are fallbacks only: an image with usable header frequency metadata should
    continue to use its own value. This is essential when one PyBDSF call mixes
    MFS, low-band and high-band products.
    """
    ...
    try:
        ...
        with fits.open(image_path, memmap=True, do_not_scale_image_data=True) as hdul:
            if hdul and _find_header_freq_hz(hdul[0].header) is not None:
                return None
    except Exception:
        # Let find_sources report unreadable FITS files. A fallback frequency
        # remains useful for readable images whose header inspection failed.
        pass
...
def compute_base_name(stem: str, prefix: Optional[str]) -> str:
    ...
    if not base:
        base = "image"
    if prefix:
        base = re.sub(r"[^A-Za-z0-9._-]+", "_", f"{prefix}{base}").strip("._-")
    return base[:120]
